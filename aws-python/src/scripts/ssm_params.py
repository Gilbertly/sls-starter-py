...
def ssm_get_params_by_path(ssm_client, param_path, next_token=""):
    ...
    try:
        if next_token == "":
            ...
            response = ssm_client.get_parameters_by_path(
                Path=param_path, Recursive=True, MaxResults=10, NextToken=next_token
            )
        return response
    except Exception as error:
...
def exec_cli_command(
    ssm_client, ssm_filepath, action, param_path=None, parameters=[], next_token=""
):
    ...
    if action in ["update", "delete"]:
        ...
        if "NextToken" in response:
            ...
            exec_cli_command(
                ssm_client, ssm_filepath, action, param_path, parameters, next_token
            )
        else:
            parameters += response["Parameters"]
            ...
            with open(ssm_filepath, "w", encoding="utf-8") as json_file:
                json.dump(
                    all_parameters, json_file, indent=2, sort_keys=True, default=str
                )
            print(
...
if __name__ == "__main__":
    ...
    try:
        if action.lower() == "download":
            param_path = input(
                "Enter ssm parameter path to download from [eg. '/kickoffai/dataml']: "
            )
            ssm_filepath = join(dirname(__file__), f"../../config/ssm.download.json")

        elif action.lower() == "update":
            param_path = input(
                "Enter ssm parameters' filename to update from [eg. ssm.<filename>.json]: "
            )
            ssm_filepath = join(
            ...
            param_path = input(
                "Enter ssm parameters' filename to delete from [eg. ssm.<filename>.json]: "
            )
            ssm_filepath = join(
                dirname(__file__), f"../../config/ssm.{param_path}.json"
            )
